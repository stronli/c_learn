**r C++ 标准模板库 (STL, Standard Template Library)**：包含一些常用数据结构与算法的模板的 C++ 软件库。其包含四个组件——算法 (Algorithms)、容器 (Containers)、仿函数 (Functors)、迭代器 (Iterators).
...
有些情况普通数组没法解决：$n\times m$ 的矩阵， 
$$
$1\leq n,m\leq 10^6$ 且 $n\times m \leq 10^6$
$$
...
例如：
$$
$[1,1,4,5,1,4]\to[1,4,5,1,4,\underline?]$
$$
，下划线位置为返回的迭代器指向。
